...
- Action taken: after the live discrepancy was observed, the exact sitemap URL was purged through Cloudflare Custom Purge. Google Search Console URL Inspection was then run again at 11:00 AM and its tested-page source contained all 26 current URLs plus the closing XML tag. This confirms Google’s live tester can retrieve the complete current document; it does not yet mean that the Sitemaps report has finished processing it.
